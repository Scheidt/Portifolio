...
// 1. DICIONÁRIO DE DADOS (Configuração Centralizada)
const CV_DATA = {
  header: {
...
const SkillCard = ({ data }) => (
  ...
  >
    ...
    {data.university && (
      <div className="mb-3">
        <Text type="secondary" className="text-[10px] uppercase font-bold block mb-1 tracking-wider">
          Aulas da Faculdade
        </Text>
        <ul className="list-disc list-inside text-sm text-gray-600 leading-relaxed">
          {data.university.map((item, i) => <li key={i}>{item}</li>)}
        </ul>
      </div>
    )}
    ...
    {data.extra && (
      <div className="mb-3">
        ...
        <Paragraph className="text-sm italic text-gray-700 m-0">{data.extra}</Paragraph>
      </div>
    )}
    ...
    {data.tags && (
      <div className="mt-4 pt-2 border-t border-gray-50">
        {data.tags.map(tag => (
          <Tag color="default" key={tag} className="mb-1 rounded-full border-gray-200 text-gray-500">
            {tag}
          </Tag>
        ))}
      </div>
    ...
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-12">
          ...
          <Text className="text-lg text-gray-500">{CV_DATA.header.subtitle}</Text>
        </header>
        ...
        <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
          <Title level={3} className="mb-8 flex items-center gap-2 text-gray-800">
            COMPETÊNCIAS
          </Title>
          <Row gutter={[40, 20]}>
            {CV_DATA.competencies.map((skill, index) => (
